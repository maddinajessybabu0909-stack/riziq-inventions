...
export function SiteHeader() {
  ...
  return (
    <header className="fixed inset-x-0 top-4 z-50">
      <nav className="site-container relative flex min-h-16 items-center justify-between rounded-lg border border-border/70 bg-card/95 shadow-nav backdrop-blur" aria-label="Main navigation">
        <Link to="/" className="shrink-0" aria-label="RIZIQ home" onClick={() => setOpen(false)}>
          <img src={horizontalLogo} alt="RIZIQ Inventions" className="h-10 w-auto object-contain object-left md:h-11" />
        </Link>
...
export function SiteFooter() {
  return (
    <footer className="bg-background pb-4 pt-12 md:pb-6">
      <div className="footer-panel site-container relative overflow-hidden rounded-lg py-8 text-footer-foreground shadow-footer md:py-10">
        ...
        <div className="grid gap-10 border-b border-footer-border pb-9 lg:grid-cols-[1.35fr_1fr_1fr]">
          <div>
            <Link to="/" className="inline-flex" aria-label="RIZIQ home">
              <img src={horizontalLogo} alt="RIZIQ Inventions" className="h-11 w-auto object-contain object-left" />
            </Link>
